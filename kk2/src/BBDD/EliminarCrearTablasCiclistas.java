...
public class EliminarCrearTablasCiclistas {
	...
	private static String nombreEquipos[] = new String[] {"Fnatic", "Origen", "G2", "Movistar Riders"};
	...
	private static int REGISTROS_A_INSERTAR_EN_CICLISTAS = 200;
	 ...
	 * 
	 */
	private static void creacionDatosCiclistas (Connection conn) throws SQLException, ImposibleConectarException {
		Statement s = (Statement) conn.createStatement();
		...
		if (LOG)
			System.out.println("\nInsertando registros de en la tabla ciclistas");

		
		String sql = "INSERT INTO ciclistas.ciclista (dorsal, nombre, edad, nomeq) VALUES  (?,?,?,?)";
		PreparedStatement ps= (PreparedStatement) conn.prepareStatement("INSERT INTO ciclistas.ciclista (dorsal, nombre, edad, nomeq) VALUES  (?,?,?,?)");
		
		for (int i = 0; i < REGISTROS_A_INSERTAR_EN_CICLISTAS; i++) {
			String equipo = getStringAlAzar(nombreEquipos);
			String nombre = getStringAlAzar(nombres);
			String ape = getStringAlAzar(apellidos);
			int edad=Utils.obtenerNumeroAzar(18, 40);

			ps.setInt(1, i);
			ps.setString(2, nombre+" "+ape);
			ps.setInt(3, edad);
